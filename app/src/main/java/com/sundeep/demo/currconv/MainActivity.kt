...
class MainActivity : ComponentActivity() {
    ...
    fun MainScreen(modifier: Modifier) {
        if (mainViewModel.dataloaded.collectAsState().value) {
            Timber.i("Data available for rendering")
            if (mainViewModel.curCurrencyIndex.collectAsState().value == -1) {
                val currentLocale = Locale.getDefault()
                val currency = Currency.getInstance(currentLocale)
                val currencyCode = currency.currencyCode
                val currencyIndex = mainViewModel.getCurrencyModelIndex(currencyCode)
                ...
                mainViewModel.updateCurrency(index)
            }
            MainScreenRender(
                modifier = modifier,
                currencies = mainViewModel.allCurrencies.collectAsState().value,
                curCurrencyIndex = mainViewModel.curCurrencyIndex.collectAsState().value,
                conversions = mainViewModel.conversions.collectAsState().value
            )
        } else {
